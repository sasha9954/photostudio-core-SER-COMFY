...
function extractScenarioPipelineStoryboardOut({ sourceNode = null, sourceHandle = "" } = {}) {
  ...
  const storyboardOut = data?.storyboardOut;
  if (storyboardOut && typeof storyboardOut === "object" && !Array.isArray(storyboardOut)) return storyboardOut;
  const packageFinalStoryboard = data?.storyboardPackage?.final_storyboard;
  if (packageFinalStoryboard && typeof packageFinalStoryboard === "object" && !Array.isArray(packageFinalStoryboard)) return packageFinalStoryboard;
  const directorPackageFinalStoryboard = data?.directorOutput?.storyboardPackage?.final_storyboard;
  if (directorPackageFinalStoryboard && typeof directorPackageFinalStoryboard === "object" && !Array.isArray(directorPackageFinalStoryboard)) return directorPackageFinalStoryboard;
  return null;
}
...
const [activeScenarioPipelineDebugId, setActiveScenarioPipelineDebugId] = useState(null);
const [isScenarioPipelineDebugOpen, setIsScenarioPipelineDebugOpen] = useState(false);
...
const activeScenarioPipelineDebugNode = useMemo(() => {
  if (activeScenarioPipelineDebugId) return nodes.find((n) => n.id === activeScenarioPipelineDebugId && n.type === "scenarioPipelineDebug") || null;
  return nodes.find((n) => n.type === "scenarioPipelineDebug") || null;
}, [activeScenarioPipelineDebugId, nodes]);
...
const scenarioSelectedActualWorkflowMode = String(
  scenarioSelected?.probableActualWorkflowMode
  || scenarioSelected?.videoActualWorkflowMode
  || ""
).trim().toLowerCase();
const scenarioSelectedWorkflowDegradedToI2V = Boolean(
  scenarioSelected?.lipSyncDegradedToI2V
  || (scenarioSelectedActualWorkflowMode === "i2v_with_audio" && scenarioSelectedIsLipSync)
);
const scenarioSelectedWorkflowLabel = scenarioSelectedActualWorkflowMode === "i2v_with_audio"
  ? "i2v + audio"
  : (scenarioSelectedActualWorkflowMode === "real_lipsync"
    ? "real lipsync"
    : (scenarioSelectedIsLipSync ? "lip_sync_music (route)" : "standard i2v"));
...
const scenarioSelectedVideoPreviewSource = useMemo(() => {
  const candidates = [
    { field: "videoSourceImageUrl", value: scenarioSelectedVideoSourceImageUrl },
    { field: "startImageUrl", value: String(scenarioSelectedEffectiveStartImageUrl || "").trim() },
    { field: "imageUrl", value: String(scenarioSelected?.imageUrl || "").trim() },
  ];
  const hit = candidates.find((candidate) => Boolean(candidate.value));
  return {
    resolvedFrom: hit?.field || "none",
    url: resolveAssetUrl(hit?.value || ""),
    present: Boolean(hit?.value),
  };
}, [scenarioSelected?.imageUrl, scenarioSelectedEffectiveStartImageUrl, scenarioSelectedVideoSourceImageUrl]);
const scenarioSelectedVideoSourcePosterUrl = scenarioSelectedVideoPreviewSource.url;
...
const scenarioCreateButtonBusy = scenarioVideoLoading;
  ...
  useEffect(() => {
    ...
    console.info("[SCENARIO VIDEO UI DEBUG]", {
      sceneId: String(scenarioSelected?.sceneId || ""),
      videoStatus: String(scenarioSelected?.videoStatus || "").trim(),
      previewSourceResolvedFrom: scenarioSelectedVideoPreviewSource.resolvedFrom,
      previewSourceUrlPresent: scenarioSelectedVideoPreviewSource.present,
      createButtonBusyDerivedFromStatus: scenarioCreateButtonBusy,
    });
  }, [scenarioCreateButtonBusy, scenarioSelected, scenarioSelectedVideoPreviewSource.present, scenarioSelectedVideoPreviewSource.resolvedFrom]);
  ...
  const updateScenarioScene = useCallback((sceneRef, patch, options = {}) => {
    ...
    setNodes((prev) => prev.map((n) => {
      ...
      const sourceFieldsBefore = Object.fromEntries(
        SCENARIO_VIDEO_SOURCE_FIELDS.map((field) => [field, String(sceneAtIdx?.[field] || "").trim()])
      );
      ...
      const preserveSourceFields = Boolean(options?.preserveSourceFieldsInVideoActions);
      if (preserveSourceFields) {
        SCENARIO_VIDEO_SOURCE_FIELDS.forEach((field) => {
          if (!Object.prototype.hasOwnProperty.call(normalizedPatch, field)) return;
          const nextValue = String(normalizedPatch?.[field] || "").trim();
          if (!nextValue) delete normalizedPatch[field];
        });
      }
      ...
      if (preserveSourceFields) {
        const mergedPreview = { ...sceneAtIdx, ...normalizedPatch };
        const sourceFieldsAfter = Object.fromEntries(
          SCENARIO_VIDEO_SOURCE_FIELDS.map((field) => [field, String(mergedPreview?.[field] || "").trim()])
        );
        const sourceFieldsPreserved = SCENARIO_VIDEO_SOURCE_FIELDS.every((field) => {
          const before = sourceFieldsBefore[field];
          const after = sourceFieldsAfter[field];
          return !before || Boolean(after);
        });
        const clearedVideoFieldCount = SCENARIO_VIDEO_RESULT_FIELDS.filter((field) => {
          if (!Object.prototype.hasOwnProperty.call(normalizedPatch, field)) return false;
          return !String(normalizedPatch?.[field] || "").trim();
        }).length;
        console.info("[SCENARIO VIDEO ACTION PATCH]", {
          sceneId: String(sceneAtIdx?.sceneId || ""),
          actionName: actionName || "video_panel_action",
  ...
  const startScenarioVideoPolling = useCallback((jobMeta) => {
    ...
    const applyScenarioVideoPatch = (patch = {}, context = {}) => {
      ...
      if (didPatch) {
        updateScenarioScene(sceneId, normalizedPatch, {
          actionName: String(context?.actionName || "video_poll"),
          preserveSourceFieldsInVideoActions: true,
        });
      }
    ...
    const tick = async () => {
      ...
      try {
        ...
        const workflowInspectionPatch = {
          probableActualWorkflowMode: String(out?.probableActualWorkflowMode || "").trim(),
          lipSyncDegradedToI2V: Boolean(out?.lipSyncDegradedToI2V),
          lipSyncProofConfirmed: Boolean(out?.lipSyncProofConfirmed),
          proofReasonDetailed: String(out?.proofReasonDetailed || "").trim(),
        };
        const nextMeta = {
          ...activeMeta,
...
onClipSec: (nodeId, value) => {
        ...
        if (n.type === "scenarioStoryboard") {
          ...
          if (isDirectPipelineSource) {
            ...
            const sourceDataStoryboardOut = sourceData?.storyboardOut && typeof sourceData.storyboardOut === "object" && !Array.isArray(sourceData.storyboardOut)
              ...
              : null;
            const sourceDataStoryboardPackage = sourceData?.storyboardPackage && typeof sourceData.storyboardPackage === "object" && !Array.isArray(sourceData.storyboardPackage)
              ? sourceData.storyboardPackage
              : null;
            const sourceDataDirectorStoryboardPackage = sourceData?.directorOutput?.storyboardPackage
              ...
              : null;
            const finalStoryboard = sourceDataStoryboardPackage?.final_storyboard && typeof sourceDataStoryboardPackage.final_storyboard === "object" && !Array.isArray(sourceDataStoryboardPackage.final_storyboard)
              ? sourceDataStoryboardPackage.final_storyboard
              : (sourceDataDirectorStoryboardPackage?.final_storyboard && typeof sourceDataDirectorStoryboardPackage.final_storyboard === "object" && !Array.isArray(sourceDataDirectorStoryboardPackage.final_storyboard)
                ? sourceDataDirectorStoryboardPackage.final_storyboard
                : null);
            ...
            if (currentNormalizedPackageSceneCount === 0) {
              console.debug("[SCENARIO HANDOFF EMPTY SCENES]", {
                sourceNodeType: String(sourceNode?.type || ""),
                sourceHandle,
                hasDataStoryboardOut: !!sourceDataStoryboardOut,
                hasStoryboardPackage: !!(sourceDataStoryboardPackage || sourceDataDirectorStoryboardPackage),
        ...
        if (n.type === "scenarioPipelineDebug") {
          const nodesById = new Map((Array.isArray(effectiveNodes) ? effectiveNodes : []).map((nodeItem) => [nodeItem.id, nodeItem]));
          const incomingEdge = [...effectiveEdges]
            .reverse()
            .find((edge) => edge?.target === n.id && String(edge?.targetHandle || "") === "scenario_pipeline_debug_in") || null;
          const sourceNode = incomingEdge?.source ? (nodesById.get(incomingEdge.source) || null) : null;
          const sourceHandle = String(incomingEdge?.sourceHandle || "");
          const validScenarioSource = sourceNode?.type === "comfyNarrative" && sourceHandle === "storyboard_out";
          const connectedInputs = sourceNode?.data?.connectedInputs && typeof sourceNode.data.connectedInputs === "object" ? sourceNode.data.connectedInputs : {};
          const refsByRole = sourceNode?.data?.refsByRole && typeof sourceNode.data.refsByRole === "object" ? sourceNode.data.refsByRole : {};
          const roleTypeByRole = sourceNode?.data?.roleTypeByRole && typeof sourceNode.data.roleTypeByRole === "object" ? sourceNode.data.roleTypeByRole : {};
          const contextRefsCount = Object.values(connectedInputs).filter((item) => {
            if (!item || typeof item !== "object") return false;
            return !!String(item.value || item.url || item.preview || "").trim();
          }).length;
          const rolesCount = Object.keys(roleTypeByRole).length || Object.keys(refsByRole).length;
          ...
          return {
            ...
            data: {
              ...
              contextSummary: {
                ...
                directorNote: String(sourceNode?.data?.directorNote || "").trim(),
                director_controls: sourceNode?.data?.director_controls || {},
                contentType: String(sourceNode?.data?.contentType || "").trim(),
                format: String(sourceNode?.data?.format || "").trim(),
                context_refs: connectedInputs,
                roleTypeByRole,
                refsByRole,
                selectedCharacterRefUrl: String(sourceNode?.data?.selectedCharacterRefUrl || "").trim(),
                selectedStyleRefUrl: String(sourceNode?.data?.selectedStyleRefUrl || "").trim(),
                selectedLocationRefUrl: String(sourceNode?.data?.selectedLocationRefUrl || "").trim(),
                selectedPropsRefUrls: Array.isArray(sourceNode?.data?.selectedPropsRefUrls) ? sourceNode.data.selectedPropsRefUrls : [],
              ...
              executedStages: Array.isArray(directorOutput?.executedStages)
                ...
                : (Array.isArray(base?.data?.executedStages) ? base.data.executedStages : []),
              diagnostics,
              stageStatuses,
              onOpenScenarioPipelineDebug: onOpenScenarioPipelineDebug,
              onScenarioPipelineRunStage: async (nodeId, options = {}) => {
                ...
                const edgeToDebug = [...activeEdges]
                  .reverse()
                  .find((edge) => edge?.target === nodeId && String(edge?.targetHandle || "") === "scenario_pipeline_debug_in") || null;
                ...
                const payload = buildScenarioStageManualPayload({
                  sourceState: source?.data || {},
                  targetState: debugNode?.data || {},
                  stageId,
                  autoRun,
...
const hydrate = useCallback((source = "unknown") => {
    ...
    try {
      ...
      const droppedLegacyNodeIds = new Set(
        savedNodes
          .filter((nodeItem) => REMOVED_LEGACY_NODE_TYPES.has(String(nodeItem?.type || "")))
      ...
      const cleanEdges = savedEdges
        ...
        .filter((e) => !droppedLegacyNodeIds.has(String(e?.source || "")) && !droppedLegacyNodeIds.has(String(e?.target || "")))
      ...
      if (droppedLegacyNodeIds.size > 0) {
        console.info("[CLIP HYDRATE] dropped legacy nodes", {
          droppedNodeCount: droppedLegacyNodeIds.size,
          droppedNodeIds: Array.from(droppedLegacyNodeIds),
        });
      }
    ...
    (params) => {
      setEdges((eds) => {
        ...
        if (src.type === "comfyNarrative" && (params.sourceHandle || "") === "storyboard_out") {
          ...
          if (!isScenarioStoryboardRoute && !isScenarioPipelineDebugRoute) {
          ...
          refreshNodeBindingsForEdges(nextEdges, isScenarioStoryboardRoute
            ...
            : "edges:connect:scenario-pipeline-debug");
          return nextEdges;
        }
        ...
        if (dst.type === "scenarioStoryboard" || dst.type === "scenarioPipelineDebug") {
          traceScenarioGraphConnect("rejected", {
            sourceType: src.type,
            sourceHandle: params.sourceHandle || "",
            targetType: dst.type,
        ...
        if ((src.type === 'comfyVideoPreview' || dst.type === 'comfyVideoPreview') && (src.type !== 'comfyNarrative' && dst.type !== 'comfyNarrative')) {
          return eds;
        }
  ...
  return (
    <div className="clipSB_root">
                  ...
                  <>
                      ...
                      <div className="clipSB_audioDebugSection">
                        ...
                        {scenarioSelectedAudioSliceUrl ? (
                          ...
                          <div className="clipSB_hint">
                            {scenarioSelectedIsLipSync
                              ? (scenarioSelectedWorkflowDegradedToI2V
                                ? "Текущий workflow деградировал в i2v + audio: audioSlice используется как audio-conditioning, не как mouth lipsync."
                    ...
                    {scenarioSelectedVideoPanelActivated ? (
                      <div ref={scenarioVideoCardRef} className={`clipSB_scenarioEditRow clipSB_videoBlock${(scenarioVideoFocusPulse || scenarioVideoOpen) ? " clipSB_videoBlockPulse" : ""}`}>
                        <div className="clipSB_hint" style={{ marginBottom: 8, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                          <span>Видео сцены</span>
                          <span className="clipSB_tag">{scenarioSelectedWorkflowLabel}</span>
                          {scenarioSelectedWorkflowDegradedToI2V ? <span className="clipSB_tag" style={{ background: "#5a3d1a", color: "#ffd9a8" }}>degraded mode</span> : null}
                        </div>
                        {scenarioSelectedTransitionType === "continuous" ? (
                            ...
                            <div className="clipSB_videoResultBox">
                              <div className="clipSB_videoPreviewWrap">
                                {scenarioSelected.videoUrl ? (
                                  ...
                                  <div className="clipSB_videoFramePlaceholder">Исходное изображение для видео отсутствует</div>
                                )}
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                          ...
                          <button
                            ...
                            disabled={scenarioCreateButtonBusy || !scenarioHasImageForVideo}
                            title={scenarioSelectedIsLipSync
                              ? (scenarioSelectedWorkflowDegradedToI2V
                                ? "Текущий route lip_sync_music выполняется как i2v + audio (degraded mode), без mouth-control lipsync."
                                : "Для lipSync audioSlice будет автоматически подготовлен перед генерацией видео.")
                              : ""}
                          >
                            {scenarioCreateButtonBusy ? "Делаю..." : "Сделать видео"}
                          </button>
      ...
      <ScenarioPipelineDebugEditor
        ...
        executedStages={activeScenarioPipelineDebugNode?.data?.executedStages || []}
        onRunPipelineStage={activeScenarioPipelineDebugNode?.data?.onScenarioPipelineRunStage}
      />
